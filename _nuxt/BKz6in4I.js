import{m as y,r as g,z as x,n as M,p as C,q as F,o as h,c as P,y as k}from"./Dc6rEite.js";import{V as w,W as U,P as L,S as R,h as V,a as b,b as T,e as G,D as W,M as D,l as O,g as B,C as E}from"./Dmytbkr3.js";import{G as j}from"./hsJpI9MV.js";import{u as I}from"./C7vw3ofb.js";const q=`#define GLSLIFY 1
uniform float time;
...
}`,Y=y({__name:"Roll",setup(z){const t=new j,i=g({angle:.15,rollOpen:()=>{x.to(a.uniforms.progress,{duration:1.7,value:1,ease:"power2.inOut"})},rollClose:()=>{x.to(a.uniforms.progress,{duration:1.7,value:.1,ease:"power2.inOut"})}}),c=g(null),e=g(new w(window.innerWidth,window.innerHeight)),_=new E;let s,u,o,m,a,r;const A=()=>{c.value&&(s=new U({logarithmicDepthBuffer:!0,canvas:c.value}),s.setSize(e.value.x,e.value.y),s.setPixelRatio(window.devicePixelRatio),s.setClearColor(16777215,1),o=new L(70,e.value.x/e.value.y,300,1e3),o.position.set(0,0,400),o.lookAt(0,0,0),u=new R,new V().load(I("/images/experiments/roll/texture.jpg"),n=>{m=new b(1,1,80,80),a=new T({uniforms:{time:{value:0},progress:{value:.2},angle:{value:.15},resolution:{value:new G(n.image.width,n.image.height,1,1)},uTexture:{value:n}},transparent:!0,side:W,vertexShader:H,fragmentShader:q}),console.log(a),r=new D(m,a),u.add(r),t.add(i.value,"angle",0,1,.01),t.add(i.value,"rollOpen"),t.add(i.value,"rollClose"),x.to(a.uniforms.progress,{duration:1.7,value:1,ease:"power2.inOut"}),v()}))},d=()=>{requestAnimationFrame(d);const n=_.getElapsedTime();a&&(a.uniforms.time.value=n,a.uniforms.angle.value=i.value.angle),s.render(u,o)},v=()=>{e.value.set(window.innerWidth,window.innerHeight),o.aspect=e.value.x/e.value.y,o.updateProjectionMatrix(),s.setSize(e.value.x,e.value.y);const n=O.degToRad(o.fov),f=Math.abs((o.position.z-r.position.z)*Math.tan(n/2)*2),S=f*o.aspect,p=new w,l=new B;p.set(e.value.x>e.value.y?e.value.y*.5:e.value.x*.1,e.value.x>e.value.y?e.value.y*.5:e.value.y*.333),l.set(S*(e.value.x-p.x)/e.value.x,f*(e.value.y-p.y)/e.value.y,1),r.scale.set(l.x,l.y,l.z)};return M(()=>{C(()=>{A(),d(),window.addEventListener("resize",v)})}),F(()=>{s.dispose(),window.removeEventListener("resize",v)}),(n,f)=>(h(),P("canvas",{ref_key:"rollRef",ref:c},null,512))}}),Q=y({__name:"Roll",setup(z){return(t,i)=>(h(),k(Y))}});export{Q as default};
