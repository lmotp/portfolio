...
const init = () => {
  if (!cardWrapRef.value || !svgRef.value) return;
  ...
  const pathElement = svgRef.value?.querySelector<SVGPathElement>(".path");
  const svgLegnth = pathElement?.getTotalLength() || 0;
  ...
  svgTl.fromTo(
    pathElement,
    {
      strokeDashoffset: svgLegnth * 0.96,
      strokeDasharray: `${svgLegnth} ${svgLegnth}`,
    },
    {
      strokeDashoffset: 0,
    }
  ...
  bgTl.fromTo(".skills", { backgroundColor: "#b7b7b7" }, { backgroundColor: "#e8e8e8" }, 0);
  bgTl.fromTo(".card-wrap", { yPercent: 35 }, { yPercent: 0 }, 0);

  cards.forEach((card, index) => {
    gsap.to(card, {
      y: gsap.utils.wrap([-9, 9]),
  ...
  tl.to(cards, { rotateZ: 0, rotateY: 0, stagger: 0.3, ease: "back.out", duration: 5 });
};
...
<template>
  <div class="skills">
    <div class="svg-wrap">
      <svg ref="svgRef" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920 3203">
        <path
          class="path"
